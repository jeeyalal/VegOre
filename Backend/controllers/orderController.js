...
export const createOrder = async (req, res) => {
  try {
    ...
    if (!address || !address.line1 || !address.city || !name || !email || !phone) {
      return res
        .status(400)
        .json({ success: false, message: "User address & contact required" });
    }
...
export const getOrdersForUser = async (req, res) => {
  try {
    ...
    const orders = await OrderModel.find({ user: decoded.id }).sort({
      createdAt: -1,
    });
