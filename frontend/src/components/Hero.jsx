...
//     subtitle: "Get personalized meal & health insights in minutes",
//     btn: "Check My Health",
//     gradient: "from-teal-600 via-emerald-700 to-green-800",
//     img: "/images/health.png", // you can change image
//     accent: "bg-lime-300",

//     // 👇 NEW FIELD: clicking this should redirect to your Google Form
//     link: "https://docs.google.com/forms/d/your-form-id-here",
//   },
// ];
//   const nextSlide = () => {
//     setCurrentSlide((prev) => (prev + 1) % slides.length);
...
//                     src={slide.img}
//                     alt={slide.title}
//                     className="w-45 sm:w-48 md:w-64 drop-shadow-2xl"
//                   />
//                 </div>
      ...
      >
        ...
        <div className="relative w-full">
          {slides.map((slide, index) => (
            ...
            >
              <div
                className={`w-full min-h-[350px] sm:min-h-[400px] md:min-h-[500px] 
                p-6 sm:p-8 md:p-10 pb-12 flex flex-col justify-center items-center 
                bg-gradient-to-br ${slide.gradient} text-white relative overflow-hidden`}
              >
                {/* Background Circles */}
                <div className="absolute top-0 right-0 w-52 sm:w-72 h-52 sm:h-72 bg-white opacity-[0.06] rounded-full translate-x-20 -translate-y-20 blur-xl"></div>
                <div className="absolute bottom-0 left-0 w-72 sm:w-96 h-72 sm:h-96 bg-white opacity-[0.06] rounded-full -translate-x-32 translate-y-32 blur-xl"></div>
                ...
                {/* Image */}
                <div className="relative z-10 mb-3 sm:mb-4 hover:scale-105 transition-transform duration-500 flex-shrink-0">
                  <img
                    src={slide.img}
                    alt={slide.title}
                    className="w-32 sm:w-40 md:w-48 lg:w-56 h-auto drop-shadow-2xl"
                  />
                </div>

                {/* Title */}
                <h2 className="text-2xl sm:text-4xl md:text-5xl lg:text-4xl font-bold text-center drop-shadow-xl px-4 leading-tight">
                  {slide.title}
                </h2>

                {/* Subtitle */}
                <p className="text-xs sm:text-sm md:text-base lg:text-lg opacity-95 mt-2 sm:mt-3 text-center max-w-xs sm:max-w-md px-4 leading-snug">
                  {slide.subtitle}
                </p>

                {/* Button */}
                <a
                  href={slide.link || "/subscription"}
                  className={`mt-5 sm:mt-6 px-5 sm:px-15 md:px-17 py-2 sm:py-2.5 md:py-3 
                  ${slide.accent} text-green-900 rounded-full text-lg sm:text-sm md:text-base 
                  lg:text-lg font-bold shadow-xl hover:shadow-2xl hover:scale-105 transition-all`}
                >
                  {slide.btn}
                </a>
              </div>
